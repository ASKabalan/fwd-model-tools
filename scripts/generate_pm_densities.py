...
def parse_args():
    ...
    parser.add_argument(
        "--omega-b", type=float, default=0.049, help="Baryon density Omega_b"
    )
    parser.add_argument(
        "--h",
        type=float,
    ...
    parser.add_argument(
        "--n-s", type=float, default=0.9665, help="Scalar spectral index n_s"
    )
    parser.add_argument(
        "--w0",
        type=float,
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for initial conditions"
    )
...
def compute_snapshot_times(args, cosmo, temp_field):
    ...
    if args.redshifts is not None:
        ...
        a_center = compute_snapshot_scale_factors(
            cosmo, temp_field, nb_shells=args.nb_shells
        )
...
def main():
    ...
    if args.box_size is not None:
        ...
        box_size = compute_box_size_from_redshift(
            cosmo, args.max_redshift, observer_position
        )
    ...
    drift, kick = symplectic_ode(
        mesh_size, paint_absolute_pos=False, halo_size=0, sharding=None
    )
